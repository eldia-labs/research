...
async function fetchCrossRef(doi: string): Promise<PaperMetadata | null> {
  try {
    ...
    const title = work.title?.[0] ?? work["short-title"]?.[0] ?? "Untitled";
...
async function fetchArxiv(arxivId: string): Promise<PaperMetadata | null> {
  try {
    ...
    const year = published
      ...
      : undefined;
    const abs =
      decodeXml(extractFirstTag(xml, "summary") ?? "").trim() || undefined;
    const idUrl =
      decodeXml(extractFirstTag(xml, "id") ?? "").trim() || undefined;
