...
export function PdfViewer({ file, onSelection }: PdfViewerProps) {
  ...
  const singlePageWidthPx = useMemo(() => {
    ...
    const heightBasedWidth = Math.max(
      Math.round((viewerHeight - 24) * PDF_PAGE_RATIO),
      120,
    );
  ...
  const handleLoadSuccess = ({
    ...
    setPageNumber((currentPage) =>
      Math.min(Math.max(currentPage, 1), loadedPages),
    );
  };
        ...
        <div className="mx-auto w-full max-w-full p-3">
          <div className="flex flex-col items-center gap-3">
            <ReactPdfDocument
              ...
              loading={
                <p className="text-muted-foreground pt-12 text-sm">
                  Loading PDF…
                </p>
              }
              error={
                <p className="text-destructive pt-12 text-sm">
                  Failed to load PDF.
                </p>
              }
            >
